            ...
            <form action="{{ route('savePerson') }}" method="POST" class="form shadow-lg rounded p-4">
                        ...
                        @isset($document_types)
                            @foreach ($document_types as $document_type)
                                <option value="{{ $document_type->id }}"
                                    @isset($person)
                                    @selected(old('document_type_id', $person) == $person->document_type->id)
                                @endisset>
                ...
                <div class="form-outline mb-3">
                    <div class="form-group">
                        ...
                        <input type="text" name="document_number" id="document_number" class="form-control w-100"
                            value="{{ old('document_number', $person) }}" />
                    </div>
                        ...
                        @isset($departments)
                            @foreach ($departments as $department)
                                <option value="{{ $department->id }}"
                                    @isset($person)
                                    @selected(old('department_id', $person) == $person->department->id)
                                @endisset>
                        ...
                        @isset($municipalities)
                            @foreach ($municipalities as $municipality)
                                <option value="{{ $municipality->id }}"
                                    @isset($person)
                                    @selected(old('municipality_id', $person) == $person->municipality->id)
                                @endisset>
                ...
                <div class="form-outline mb-3">
                    ...
                    <select id="zone_type" name="zone_type" class="form-control form-select w-100">
                        ...
                        <option value="Urbana"
                            @isset($person)
                                {{ $person->state == 'Urbana' ? 'selected' : '' }}
                            @endisset>
                            Urbana</option>

                        <option value="Rural"
                            @isset($person)
                                {{ $person->state == 'Rural' ? 'selected' : '' }}
                            @endisset>
                            Rural</option>
                    </select>
